...
class System extends HTTP {
  ...
  public async plotter(systemA: string, systemB: string, params: object = {}): Promise<any> {
    const p = { ...params, start: systemA, end: systemB }
    const response = await this.http.get('/systems/plotter', {
      params: p,
    })
    return response.data
  }
