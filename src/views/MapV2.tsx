...
const MapV2: FC = () => {
  ...
  const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.value.length > 0) {
      ...
      const i = dataset.metadata.reduce((a: number[], c, i) => {
        if (c.label?.split(' | ')[0].toLocaleLowerCase().indexOf(s) !== -1) {
          a.push(i)
        }
        return a
      }, [])
    ...
    <div className="relative flex w-full min-h-full">
      <div className="absolute left-8 right-8">
        <div className="w-full p-4 mt-20 text-gray-400 bg-gray-800 border border-gray-700 rounded-lg shadow-md md:w-3/12">
          ...
          <div className="relative pt-1">
            <label htmlFor="search" className="font-bold text-gray-400">
              Search
            </label>
            <input
              type="text"
