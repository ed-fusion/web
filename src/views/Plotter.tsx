...
import { ScatterGL, Point3D, PointMetadata, Dataset, RenderMode, Sequence } from 'scatter-gl'
...
const Plotter: FC = () => {
  ...
  const [indicies, setIndicies] = useState<number[]>([])
  const [firstRender, setFirstRender] = useState(true)
  const fetchData = async () => {
    const res = await System.plotter('Sol', 'Kokary')
    setSystems(res.route)
  }
  ...
  useEffect(() => {
    if (Object.keys(dataset).length > 0) {
      ...
      scatterGL = new ScatterGL(containerElement, opt)
  ...
  useEffect(() => {
    if (Object.keys(systems).length > 0) {
      const data: Data = {
        labelNames: systems.map((s) => `${s.name} | (${s.position.split(';').join(' / ')})`),
